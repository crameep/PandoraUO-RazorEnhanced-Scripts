...
class DurabilityChecker(object):    
    ...
    def repairshit(self,item,bench):
...
class dexxer(Form):
    ...
    def btnStartPressed(self, sender, args):
        ...
        def rail():
            if self.textbox3.Text.isdigit(): 
                chasetimer = self.textbox3.Text
            else:
                chasetimer = 0
            while Misc.ReadSharedValue('chase') == 'True' and Misc.ReadSharedValue('rail') == 'False':
                ...
                if Misc.ReadSharedValue('run') == 'False': 
                    break
            if Misc.ReadSharedValue('rail') == 'True':
                self.ReadRail()
    ...
    def btnRepairPressed(self, sender, args):
        Misc.SetSharedValue('dur',self.textbox4.Text)
        Misc.SendMessage('Durability check set at {}'.format(self.textbox4.Text),78)
    ...
    def ReadRail(self):
        #CP
        #railfile = os.path.join(os.environ.get('userprofile', 'c:'), 'Documents', 'RAIL.txt')
        railfile = "/".join([getenv('userprofile'), 'Documents', '{}.txt'.format(self.textbox2.Text)])
        ...
        if Timer.Check('chasetimer') == False:          
            ...
            if mob:                                    
                ...
                if not Player.InRangeMobile(mob,1):
                    ...
                    Timer.Create('chasetimer',chasetimer)
                else:
                    while Mobiles.FindBySerial(mob.Serial) != None:
                        Misc.Pause(2000)
                        go(mob.Position.X,mob.Position.Y)
                        if Misc.ReadSharedValue('run') == 'False':
                            break
